...
import ShippingFee, {
  transNumberFormatter,
} from "../../../components/Payment/ShippingFee";
...
const Payment = () => {
  ...
  useEffect(() => {
    if (getUserAddress.isSuccess) {
      const data = getUserAddress.data.data.find(
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.totalPriceContainer}>
          <View>
            <Text
              style={styles.label}
            >{`Tổng số tiền (${totalProd} sản phẩm):`}</Text>
          </View>
          <Text style={styles.totalPrice}>
            <Text style={styles.vndText}>đ</Text>
            {transNumberFormatter(10000)}
          </Text>
        </View>
